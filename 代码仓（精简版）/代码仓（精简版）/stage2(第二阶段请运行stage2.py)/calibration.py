...
# from ..src.utils.utils import set_seed, load_checkpoint, save_checkpoint, collate, copy_fn, AverageMeter, count_parameters, check_dirs, create_logger
...
def metric(test_logits, test_labels, logits, labels):
    ...
    print('Before temperature - ACC: %.2f, ECE: %.2f, NLL: %.3f, Brier: %.3f'
          % (
              before_temperature_acc * 100, before_temperature_ece * 100, before_temperature_nll,
              before_temperature_brier))
...
def adaptive_ts(train_datas, test_datas, val_datas=None, args=None):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for step, data in enumerate(train_loader):  #
            ...
            optimizer.step()
            optimizer.zero_grad()
        val_loss = F.cross_entropy(model(train_logits.to(args.device)), train_labels.to(args.device))
