...
# from src.models import get_model, emc_loss
...
# os.environ['OMP_NUM_THREADS'] = '1'  # 避免 OpenMP 冲突
#
# # 禁用警告
# import warnings
...
#
# # 设置环境变量（放在最前面）
# os.environ['HF_HOME'] = 'D:/models/huggingface'  # 使用HF_HOME替代TRANSFORMERS_CACHE
# os.environ['TORCH_HOME'] = 'D:/models/torch_hub'
# os.environ['OMP_NUM_THREADS'] = '1'  # 避免OpenMP冲突
#
#
# def get_args(parser):
#     parser.add_argument("--batch_sz", type=int, default=128)
...
#     parser.add_argument("--max_epochs", type=int, default=100)
#     parser.add_argument("--max_seq_len", type=int, default=512)
#     parser.add_argument("--n_workers", type=int, default=0)  # 改为0，禁用多进程
#     parser.add_argument("--name", type=str, default="nameless")
#     parser.add_argument("--num_image_embeds", type=int, default=1)
#     parser.add_argument("--patience", type=int, default=10)
#     parser.add_argument("--savedir", type=str, default="/path/to/save_dir/")
...
#                         choices=["CrisisMMD/damage", "CrisisMMD/humanitarian", "CrisisMMD/informative",
#                                  "N24News/abstract", "N24News/caption", "N24News/headline",
#                                  "food101", "MVSA_Single"])
#     parser.add_argument("--warmup", type=float, default=0.1)
#     parser.add_argument("--clip_grad", type=float, default=0)
...
#         no_decay = ["bias", "LayerNorm.bias", "LayerNorm.weight"]
#
#         lr_text_tfm = args.lr_text_enc
#         lr_img_tfm = args.lr_img_enc
#         lr_text_cls = args.lr_text_cls
#         lr_img_cls = args.lr_img_cls
#         weight_decay_tfm = args.weight_decay_enc
#         weight_decay_other = args.weight_decay_cls
#
#         optimizer_grouped_parameters = [
#             {"params": [p for n, p in text_enc_param if not any(nd in n for nd in no_decay)],
#              "weight_decay": weight_decay_tfm, 'lr': lr_text_tfm},
...
#                 "n_no_improve": n_no_improve,
#                 "best_metric": best_metric,
#                 "best_all_metric": best_metric,  # 修复未定义变量
#             },
#             is_improvement,
#             args.savedir,
#         )
...
def get_args(parser):
    ...
    parser.add_argument("--n_estimators", type=int, default=500)
    parser.add_argument("--regressor", type=int, default=0)
    parser.add_argument("--noise", type=float, default=0.0)  # 添加noise参数，避免pkl文件路径错误
...
def train(args):
    ...
    if os.path.exists(os.path.join(args.savedir, "checkpoint.pt")):
        checkpoint = torch.load(os.path.join(args.savedir, "checkpoint.pt"), weights_only=False)  # 修复加载问题
        i_epoch = start_epoch = checkpoint["epoch"]
        n_no_improve = checkpoint["n_no_improve"]
        best_metric = checkpoint["best_metric"]
        model.load_state_dict(checkpoint["state_dict"])
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        metrics = model_eval(i_epoch, val_loader, model, args, criterion, logger=logger)
        logger.info("Val  : " + logger_str(metrics))
        log_metrics("Val  ", metrics, args, logger)
    ...
    try:
        # 修复模型加载问题
        checkpoint_path = os.path.join(args.savedir, "model_best.pt")
        if os.path.exists(checkpoint_path):
            # 使用weights_only=False来避免加载错误
            checkpoint = torch.load(checkpoint_path, weights_only=False)
            model.load_state_dict(checkpoint["state_dict"])
            logger.info("✓ 成功加载最佳模型")
        else:
            logger.warning("最佳模型文件不存在，使用当前模型")
    except Exception as e:
        logger.warning(f"加载最佳模型失败: {e}, 使用当前模型进行评估")

    # 强制生成pkl文件
    logger.info("开始生成评估结果和pkl文件...")
    model.eval()

    # 训练集评估和pkl生成
    try:
        metrics = model_eval(final_epoch + 1, train_loader, model, args, criterion, logger=logger, store_preds=True,
                             type_="train")
        logger.info("Train: " + logger_str(metrics))
        log_metrics("Train", metrics, args, logger)
        train_data = metrics["data"]
        logger.info("✓ 训练集pkl文件生成成功")
    except Exception as e:
    ...
    # 验证集评估和pkl生成
    try:
        metrics = model_eval(final_epoch + 1, val_loader, model, args, criterion, logger=logger, store_preds=True,
                             type_="val")
        logger.info("Val  : " + logger_str(metrics))
        log_metrics("Val  ", metrics, args, logger)
        val_data = metrics["data"]
        logger.info("✓ 验证集pkl文件生成成功")
    except Exception as e:
    ...
    for test_name, test_loader in test_loaders.items():
        try:
            test_metrics = model_eval(final_epoch + 1, test_loader, model, args, criterion, logger=logger,
                                      store_preds=True)
            logger.info("Test : " + logger_str(test_metrics))
            log_metrics(f"Test - {test_name}", test_metrics, args, logger)
            print(logger_str(test_metrics))
            ...
            if args.regressor:
                if train_data is not None and val_data is not None:
                    # 合并训练和验证数据用于回归器
                    combined_data = {
                        "tgts": np.hstack((train_data["tgts"], val_data["tgts"])),
                        "outs": {}
                    }
                    for key in val_data["outs"].keys():
                        combined_data["outs"][key] = np.vstack((train_data["outs"][key], val_data["outs"][key]))

                    test_data = test_metrics["data"]
                    preds = model.wmodel.train_predict(combined_data, test_data)
                    preds = preds.argmax(-1)
                    acc = accuracy_score(test_data["tgts"], preds)
                    print(f"test acc: {acc:.4f}")
                else:
                    logger.warning("无法进行回归器测试，训练或验证数据缺失")
            else:
                print(f"test acc: {test_metrics['acc']:.4f}")

            logger.info(f"✓ 测试集 {test_name} pkl文件生成成功")
        except Exception as e:
            logger.error(f"测试集 {test_name} 评估失败: {e}")

    # 记录训练完成状态
    if early_stop:
        logger.info(f"训练提前终止于第 {final_epoch + 1} 个epoch")
    else:
        logger.info(f"训练完成于第 {final_epoch + 1} 个epoch")

    logger.info("所有评估和pkl文件生成完成")


def model_eval(i_epoch, data, model, args, criterion, logger=None, store_preds=False, type_="test"):
    with torch.no_grad():
        losses, preds, tgts = [], [], []
        preds_ = defaultdict(list)
        outs = defaultdict(list)
    ...
    if args.task_type == "multilabel":
        ...
        for i in preds_.keys():
            ...
            accs.append(acc_.sum() / len(acc_))
            accs_a += accs[i]
            outs[i] = np.vstack(outs[i])  # 修复：使用vstack替代row_stack
        acc_dict["MI"] = accs
        acc_dict["AM"] = accs_a
        acc_dict["MIN"] = accs_.sum() / len(accs_)
        acc_dict["MAX"] = accs_m.sum() / len(accs_m)
        ...
        save_data["outs"] = outs
        if store_preds:
            # 确保路径正确，使用getattr避免noise属性不存在
            noise = getattr(args, 'noise', 0.0)
            path = f"{args.savedir}/{args.name}_{type_}_{noise}.pkl"
            print(f"Save to {path}")
            with open(path, "wb") as f:
                pickle.dump(save_data, f)
            # 修复：检查logger是否存在
            if logger is not None:
                logger.info(f"✓ pkl文件已保存: {path}")
            else:
                print(f"✓ pkl文件已保存: {path}")
        metrics["data"] = save_data
